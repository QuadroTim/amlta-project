...
OUTPUT_FOLDER = "/Users/timskeip/Downloads/data-2/ILCD/embedding_multilingual_e5_large"
os.makedirs(OUTPUT_FOLDER, exist_ok=True)

# Загрузка модели эмбеддингов
model = SentenceTransformer('intfloat/multilingual-e5-large')  # Используем multilingual-e5-large
...
def main():
    for filename in os.listdir(DATA_FOLDER):
        if filename.endswith('.yaml'):
            ...
            # Подготовка текста
            text = prepare_text_for_rag(data, prefix="passage: ")  # Используйте "query: " или "passage: " в зависимости от задачи
            if not text.strip():
            ...
            if embedding.shape[0] != 1024:  # Размерность для multilingual-e5-large
...
# Папка с файлами
DATA_FOLDER = "/Users/timskeip/Downloads/data-2/ILCD/embedding_multilingual_e5_large"

# Список для точек Qdrant и BM25
points = []
texts = []
...
for i, filename in enumerate(os.listdir(DATA_FOLDER)):
    if filename.endswith('.txt'):
        ...
        if not os.path.exists(npy_path):
            print(f"⚠️ Эмбеддинг для {filename} не найден.")
            continue
